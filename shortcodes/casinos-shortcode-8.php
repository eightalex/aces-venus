...
function aces_casinos_shortcode_8($atts) {
	...
	<?php if ( $load_more_button ) { ?>
		...
		<style type="text/css">
			.space-organizations-7-archive-items .space-organizations-7-archive-item:nth-child(<?php echo esc_attr( $show_items ); ?>) ~ .space-organizations-7-archive-item {
			  display: none;
			}
		</style>
		<script type="text/javascript">
			jQuery(document).ready( function($){
				'use strict';
				const list = document.querySelector(".space-organizations-7-archive-items");
				const list_items = list.querySelectorAll(".space-organizations-7-archive-item");
				const ajax_load_more = document.querySelector(".shortcode-load-more-8");
				...
				ajax_load_more.addEventListener("click", function () {
					'use strict';
					let range = `.space-organizations-7-archive-item:nth-child(n+${count}):nth-child(-n+${double_count})`;
					list
			...
			<?php if ( $load_more_button ) { ?>
				...
				<div class="space-load-more-button-wrap text-center box-100 relative">
					<div class="space-load-more-button-wrap-ins relative">
						<button class="space-load-more-button shortcode-load-more-8"><?php echo esc_html__( 'Load More', 'aces' ); ?></button>
					</div>
				</div>
